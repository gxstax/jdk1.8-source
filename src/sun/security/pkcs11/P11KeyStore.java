...
final class P11KeyStore extends KeyStoreSpi {
    ...
    public synchronized void engineSetKeyEntry(String alias, Key key,
        ...
        try {
            if (key instanceof PrivateKey) {
                entry = new PrivateKeyEntry((PrivateKey)key, chain);
        ...
        engineSetEntry(alias, entry, new PasswordProtection(password));
    }
    ...
    public synchronized void engineSetCertificateEntry
        ...
        Entry entry = null;
        entry = new TrustedCertificateEntry(cert);
     ...
     */
    public synchronized void engineStore(LoadStoreParameter param)
        throws IOException, NoSuchAlgorithmException, CertificateException {
        token.ensureValid();
        if (param != null) {
     ...
     */
    public synchronized void engineLoad(LoadStoreParameter param)
     ...
     * @since 1.5
     */
    public synchronized Entry engineGetEntry(String alias,
        ...
        if (protParam != null &&
            ...
            ((PasswordProtection)protParam).getPassword() != null &&
            !token.config.getKeyStoreCompatibilityMode()) {
        ...
        try {
            ...
            if (aliasInfo.type == ATTR_CLASS_CERT) {
                ...
                return new TrustedCertificateEntry(aliasInfo.cert);
            } else if (aliasInfo.type == ATTR_CLASS_SKEY) {
                // secret key entry
                if (debug != null) {
                ...
                if (h.type != ATTR_CLASS_SKEY) {
                    ...
                    return new SecretKeyEntry(skey);
                }
            } else {
                // private key entry
                if (debug != null) {
                ...
                if (h.type != ATTR_CLASS_PKEY) {
                    ...
                    if ((pkey != null) && (chain != null)) {
                        return new PrivateKeyEntry(pkey, chain);
                    } else {
                        if (debug != null) {
                            debug.println
     ...
     * @since 1.5
     */
    public synchronized void engineSetEntry(String alias, Entry entry,
                        ProtectionParameter protParam)
                throws KeyStoreException {
            ...
            if (entry instanceof PrivateKeyEntry) {
                ...
                try {
                    ...
                    if (!updatedAlias) {
                        ...
                        storePkey(alias, (PrivateKeyEntry)entry);
                    }
    ...
    public synchronized boolean engineEntryInstanceOf
                (String alias, Class<? extends Entry> entryClass) {
        token.ensureValid();
    ...
    private void storeSkey(String alias, SecretKeyEntry ske)
                throws PKCS11Exception, KeyStoreException {
    ...
    private void storePkey(String alias, PrivateKeyEntry pke)
